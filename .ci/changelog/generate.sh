...
nightly)
	TAG="v${TIMESTAMP}.${FORGEJO_REF}"
	REF="${FORGEJO_REF}"
	BASE_DOWNLOAD_URL="https://$RELEASE_NIGHTLY_HOST/$RELEASE_NIGHTLY_REPO/releases/download"
	;;
*)
	echo "Type: $1"
...
master)
	...
	;;
nightly)
	echo "Nightly build based on [\`$FORGEJO_REF\`](https://$FORGEJO_HOST/$FORGEJO_REPO/commit/$FORGEJO_REF)"
	;;
pull_request)
...
linux_field() {
	...
	echo -n "| $PRETTY_ARCH | "
	echo -n "[Download](${BASE_DOWNLOAD_URL}/${TAG}/${PROJECT_PRETTYNAME}-Linux-${REF}-${ARCH}-gcc-standard.AppImage) "
	if tagged; then
