...
parse_payload() {
	...
	RELEASE_PGO_REPO=$(jq -r --arg id "pgo" '( .[] | select(.["build-id"] == $id) | .repository ) // "QDash-CI/PGO"' $RELEASE_JSON)
	...
	nightly)
		...
		RELEASE_NIGHTLY_HOST=$(jq -r --arg id "nightly" '.[] | select(.["build-id"] == $id) | .host' $RELEASE_JSON)
		RELEASE_NIGHTLY_REPO=$(jq -r --arg id "nightly" '.[] | select(.["build-id"] == $id) | .repository' $RELEASE_JSON)
		;;
	*)
		echo "Type: $1"
		echo "Supported types: master | pull_request | nightly | tag | push | test"
		exit 1
		;;
	esac

	{
		...
		echo "RELEASE_NIGHTLY_HOST=$RELEASE_NIGHTLY_HOST"
		echo "RELEASE_NIGHTLY_REPO=$RELEASE_NIGHTLY_REPO"
	} >> "$FORGEJO_LENV"
}
